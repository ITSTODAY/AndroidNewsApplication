...
public class ChannelAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean isRecommend = false;   //当前是否显示推荐频道
    ...
    public ChannelAdapter(Context mContext, List<ChannelBean> mList, List<ChannelBean> recommendList, List<ChannelBean> cityList, int ID) {
            ...
            public void onClick(View v) {
                if (holder.getLayoutPosition() < selectedSize + 1) {
                    ...
                    if (holder.getLayoutPosition() > fixSize) {
                        ...
                        String deleting = holder.name.getText().toString();
                        System.out.println("deleting" + " "+deleting);
                        String[] userit = user.GetCategory();
                        String[] newUserit = new String[userit.length-1];
                        int placee = 0;
                        for(String items: userit){
                            if(items.equals(deleting)){
                                continue;
                            }
                            newUserit[placee] = items;
                            placee++;
                        }

                        user.addCategory(newUserit);
                    }
                } else {
                    //tab下面的 点击添加到已选频道
                    String adding = holder.name.getText().toString();
                    String[] usedto = user.GetCategory();

                    int placee = 0;
                    String[] newone = new String[usedto.length+1];
                    for(String items:usedto){
                        newone[placee] = items;
                        placee++;
                    }
                    System.out.println("adding "+adding);
                    newone[placee] = adding;
                    System.out.println("add");
                    for(String item:newone){
                        System.out.println(item);
                    }
                    user.addCategory(newone);
                    selectedSize++;
        ...
        if (mLeft == -1) {
            holder.recommend.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    mLeft = (int) (holder.recommend.getLeft() + layout.getPrimaryHorizontal(0)) - SettingActivity.dip2px(mContext, 10);   //textView左边距离+第一个文字绘制的距离 设置了padding 所以要减掉
        ...
        holder.city.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isRecommend) {
                    ...
                    if (mRight == -1) {
                        mRight = mLeft + holder.city.getLeft() - SettingActivity.dip2px(mContext, 10);
                    }
